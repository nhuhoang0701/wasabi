...
var ASM_CONSTS = [function() {
	...
	//let fileinitial = document.getElementById('input').files[0];
	console.log("var ASM_CONSTS");
	fetch('build/MyExample.wasm')
	.then(res => res.blob()) // Gets the response and returns it as a blob
	.then(blob => {
		var f = new File([blob], "MyExample.wasm", {type:"application/wasm"});	
		//console.log(f);
		let response = new Response(f);
	
		//let file_with_mime_type = f.slice(0, f.size, 'application/wasm');
		wasi_instantiateStreaming(response, imports)
		.then(
			obj =>
			{
				setInstance(obj.instance);
				try {obj.instance.exports._start();}
				catch (e)
				{
...
async function _loadWASMFile(){
	const imports = { wasi_snapshot_preview1: WASIPolyfill };
	console.log("_loadWASMFile method")//let fileinitial = document.getElementById('input').files[0];
	await fetch('build/MyExample.wasm')
	.then(res => res.blob()) // Gets the response and returns it as a blob
	.then(blob => {
		var f = new File([blob], "MyExample.wasm", {type:"application/wasm"});	
		//console.log(f);
		...
		.then(
			obj =>
			{
				...
				catch (e)
				{
					...
					{
						console.log('error in exports._start : ' + error);
					}
				}
			})
		.catch(error => {	console.log('error! ' + error);});
