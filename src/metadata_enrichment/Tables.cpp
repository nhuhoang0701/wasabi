  ...
  namespace metadata{
    ...
    Table::Table(
                 string_view theName
                            ...
                            new utils::TableImpl(theName,theName)
                            ){
    };
    ...
    void Table::addColumn( const dbproxy::ColumnDescr& theDesc){
      ...
      auto anIter = itsPimpl->itsColumns.insert(
                                                make_pair(
                                                          theDesc.getName(),
                                                          unique_ptr<Column>(new Column(theDesc))
                                                          )
                                                );
      if(!anIter.second){
        stringstream aStream;
    ...
    void Table::write(JSONWriter& theWriter)const{
      ...
      JSON_LIST(theWriter);
      for_each(
               getColumnNames().begin(), getColumnNames().end(),
