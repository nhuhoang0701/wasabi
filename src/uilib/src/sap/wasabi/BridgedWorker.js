...
sap.ui.define(
  ...
  function(Log){
    var BuildBridgedWorker = function (
      workerFunction,
      workerExportNames,
      mainExportNames,
      mainExportHandles
    ) {
      Log.info("Creating a BridgedWorker");
      ...
      // create the worker
      var url = window.URL.createObjectURL(new window.Blob([fullWorkerStr],{type:'text/javascript'}));
      var theWorker = new window.Worker(url);
      ...
      var ret = {
        blobURL:
        url
      };//this is useful to know for debugging if you have loads of bridged workers in blobs with random names
      var makePostMessageForFunction = function(name,hasBuffers){
        if(hasBuffers) {
          return function(/*args...,[ArrayBuffer,..]*/){
            var args = Array.prototype.slice.call(arguments); var buffers = args.pop(); theWorker.postMessage({foo:name,args:args},buffers);
          };
        } else {
