...
int main()
{
	...
	{		
		...
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT dim_A, dim_B, SUM(meas_1), SUM(meas_2) FROM FAKE_TABLE_NO_DS GROUP BY dim_A, dim_B;", sql);
	}
	{
		...
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT OBJ_188, SUM(OBJ_262) FROM FAKE_TABLE_NO_DS WHERE OBJ_188 = '2014' GROUP BY OBJ_188;", sql);
	}	
	{
		...
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT OBJ_188, SUM(OBJ_262), SUM(OBJ_147) FROM FAKE_TABLE_NO_DS WHERE OBJ_188 IS NULL OR NOT ( OBJ_188 = '2014' ) GROUP BY OBJ_188;", sql);
	}	
	{
		...
		WASABI_CHECK_EQUAL(definition.getDimensions().size(),3);
		
		ina::query_model::QueryEx queryEx(definition);
		const query_generator::query_generator& queryGen = query_generator::query_generator(queryEx);
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT Yr, Month_name, SUM(Sales_revenue) FROM FAKE_TABLE_NO_DS GROUP BY Yr, Month_name;", sql);		
	}	
	{
		...
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT SUM(OBJ_147), OBJ_166, OBJ_185, OBJ_186 FROM FAKE_TABLE_NO_DS GROUP BY OBJ_166, OBJ_185, OBJ_186 ORDER BY OBJ_166 ASC, OBJ_185 DESC, OBJ_186 ASC;", sql);		
	}
	{
		...
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT SUM(Sales_revenue), SUM(Quantity_sold), SUM(Margin) FROM FAKE_TABLE_NO_DS;", sql);		
	...
	}	
	{
		...
		std::string sql = queryGen.getSQL();
		std::cout << "Generated SQL: " << sql << std::endl;
		WASABI_CHECK_EQUAL("SELECT Yr, SUM(Quantity_sold) FROM FAKE_TABLE_NO_DS GROUP BY Yr ORDER BY Yr;", sql);		
